...
import robotx_gz.bridges

import os

GYMKHANA_WORLDS = ["gymkhana_task"]
...
def simulation(world_name, headless=False, paused=False, extra_gz_args=""):
    ...
    gz_sim = IncludeLaunchDescription(
        ...
        launch_arguments={"gz_args": " ".join(gz_args), "gz_version": "8"}.items(),
    )
    ...
    p = os.path.join(get_package_share_directory("robotx_gazebo_worlds"), "launch", "monitor_sim.py")
...
def competition_bridges(world_name, competition_mode=False):
    bridges = [
        robotx_gz.bridges.clock(),
        robotx_gz.bridges.task_info(),
    ]

    if not competition_mode:
        bridges.extend(
            [robotx_gz.bridges.usv_wind_speed(), robotx_gz.bridges.usv_wind_direction()]
        )

    task_bridges = []
    if world_name in PERCEPTION_WORLDS:
        task_bridges = [
            robotx_gz.bridges.perception_reports(),
        ]
    elif world_name in STATIONKEEPING_WORLDS:
        task_bridges = [
            robotx_gz.bridges.stationkeeping_goal(),
        ]
        if not competition_mode:
            task_bridges.extend(
                [
                    robotx_gz.bridges.stationkeeping_mean_pose_error(),
                    robotx_gz.bridges.stationkeeping_pose_error(),
                ]
            )
    elif world_name in WAYFINDING_WORLDS:
        task_bridges = [
            robotx_gz.bridges.wayfinding_waypoints(),
        ]
        if not competition_mode:
            task_bridges.extend(
                [
                    robotx_gz.bridges.wayfinding_mean_error(),
                    robotx_gz.bridges.wayfinding_min_errors(),
                ]
            )
    elif world_name in GYMKHANA_WORLDS:
        task_bridges = [
            robotx_gz.bridges.gymkhana_blackbox_goal(),
        ]
        if not competition_mode:
            task_bridges.extend(
                [
                    robotx_gz.bridges.gymkhana_blackbox_mean_pose_error(),
                    robotx_gz.bridges.gymkhana_blackbox_pose_error(),
                ]
            )
    elif world_name in WILDLIFE_WORLDS:
        task_bridges = [
            robotx_gz.bridges.animal_pose("/vrx/wildlife/animal0/pose"),
            robotx_gz.bridges.animal_pose("/vrx/wildlife/animal1/pose"),
            robotx_gz.bridges.animal_pose("/vrx/wildlife/animal2/pose"),
        ]
    elif world_name in SCAN_DOCK_DELIVER_WORLDS:
        task_bridges = [
            robotx_gz.bridges.color_sequence_reports(),
        ]
    elif world_name in AUV4_WORLDS:
        task_bridges = []
...
def spawn(sim_mode, world_name, models, robot=None):
    ...
    launch_processes = []
    for model in models:
        print(f"spawn {model} {robot}")
        if robot and model.model_name != robot:
            print("invalid!!", robot, model.model_name)
            continue

        print(model, sim_mode)
        ...
        if sim_mode == "full" or sim_mode == "bridge":
            ...
            if world_name == "robotx_2024_course_docking":
                for _model_name in ["dock", "light_buoy"]:
                    bridges.extend(
                        [
                            robotx_gz.bridges.robotx_obstacle_pose(_model_name),
                        ]
                    )
            ...
            bridges.extend([robotx_gz.bridges.set_pose("vessel_1")])
            nodes.append(
                Node(
                    package="ros_gz_bridge",
                    executable="parameter_bridge",
            ...
            print(
                f"arguments: {[bridge.argument() for bridge in bridges]}\nremappings: {[bridge.remapping() for bridge in bridges]}"
            )

            # tf broadcaster (sensors)
            nodes.append(
                Node(
                    package="robotx_gazebo_worlds",
            ...
            model_dir = os.path.join(
                get_package_share_directory("robotx_gazebo_worlds"),
                f"models/{robot}/tmp",
            )
            ...
            nodes.append(
                Node(
                    ...
                    output="both",
                    parameters=[params],
                    remappings=[("/joint_states", f"/{robot}/joint_states")],
                )
            )

            group_action = GroupAction([PushRosNamespace(model.model_name), *nodes])
