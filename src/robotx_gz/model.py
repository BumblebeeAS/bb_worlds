...
import robotx_gz.payload_bridges
import pathlib
import re
import yaml
...
WAVEFIELD_SIZE = {'sydney_regatta': 1000, 'nathan_benderson_park': 1000}
...
class Model:
    ...
    def is_AUV(self):
        return self.model_type in AUVS
    ...
    def bridges(self, world_name):
        ...
        bridges = [
            # pose
            robotx_gz.bridges.pose(self.model_name),
            # pose static
            robotx_gz.bridges.pose_static(self.model_name),
            # joint states
            robotx_gz.bridges.joint_states(world_name, self.model_name),
            # comms tx
            # robotx_gz.bridges.comms_tx(self.model_name),
            # comms rx
            # robotx_gz.bridges.comms_rx(self.model_name),
        ]
        if self.is_UAV():
            bridges.extend([
                # Magnetometer
                robotx_gz.bridges.magnetometer(world_name, self.model_name),
                # Air Pressure
                robotx_gz.bridges.air_pressure(world_name, self.model_name),
            ])
            bridges.extend([
                # twist
                robotx_gz.bridges.cmd_vel(self.model_name)
            ])
        elif self.is_AUV():
            bridges.extend([
                # Magnetometer
                robotx_gz.bridges.magnetometer(world_name, self.model_name),
                # Air Pressure
                robotx_gz.bridges.water_pressure(world_name, self.model_name),
            ])
            bridges.extend([
                # twist
                robotx_gz.bridges.cmd_vel(self.model_name)
            ...
            for i in range(1, 8):
                bridges.append(robotx_gz.bridges.auv_thruster(self.model_name, i))
        elif self.is_USV():
            pass
    ...
    def payload_bridges_impl(self, world_name, payloads):
        ...
        for sensor_name, value in payloads.items():
            ...
            bridges.extend(
                robotx_gz.payload_bridges.payload_bridges(
                    world_name, self.model_name, link_name,
    ...
    def generate(self):
        print('generating urdf {} {}'.format(self.model_name, self.urdf))
        command = None
        if not self.urdf:
        ...
        if self.urdf != '':
            print('generated urdf: ' + self.urdf)
    ...
    def payload_from_sdf(self, model_sdf):
        ...
        for link_index in range(model.link_count()):
            ...
            for sensor_index in range(link.sensor_count()):
                ...
                print(f"link name: {link.name()} type: {sensor.type()}")
                payload[sensor.name()] = [link.name(), sensor.type()]
        plugins = model.plugins()
        for plugin in plugins:
            print(f"plugin name: {plugin.name()} filename: {plugin.filename()}")
            if plugin.name() == 'gz::sim::systems::Thruster':
                name = self.name_from_plugin(plugin.__str__())
                print(plugin.filename(), plugin.name(), name)
                payload['thruster_thrust_' + name] = [plugin.name(), name]
            elif plugin.name() == 'gz::sim::systems::JointPositionController':
                name = self.name_from_plugin(plugin.__str__())
                if not "thruster" in name:
                    continue
                print(plugin.filename(), name)
                payload['thruster_rotate_' + name] = [plugin.name(), name]
            elif plugin.name() == 'gz::sim::systems::OdometryPublisher':
                name = self.name_from_plugin(plugin.__str__())
                print(plugin.filename(), name)
                payload['odometry_' + name] = [plugin.name(), name]
            else:
                print(plugin.filename(), plugin.name())
                payload[plugin.name()] = [plugin.name(), plugin.filename()]
        return payload
