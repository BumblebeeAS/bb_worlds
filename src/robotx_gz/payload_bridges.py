from robotx_gz.bridge import Bridge, BridgeDirection
...
def imu(world_name, model_name, link_name, sensor_name, is_ned):
    ...
    return Bridge(
        ...
        ros_topic=f'{ros_sensor_prefix}/data{"_ned" if is_ned else ""}',
        gz_type='gz.msgs.IMU',
...
def navsat(world_name, model_name, link_name, sensor_name):
    ...
    return Bridge(
        ...
        ros_topic=f'{ros_sensor_prefix}/nav_sat_fix',
        gz_type='gz.msgs.NavSat',
        ros_type='sensor_msgs/msg/NavSatFix',
        direction=BridgeDirection.GZ_TO_ROS)

def dvl(world_name, model_name, link_name, sensor_name):
    gz_sensor_prefix = gz_prefix(world_name, model_name, link_name, sensor_name)
    ros_sensor_prefix = ros_prefix(sensor_name, "dvl")
    return Bridge(
        gz_topic=f'{gz_sensor_prefix}/dvl',
        ros_topic=f'{ros_sensor_prefix}/dvl',
        gz_type='gz.msgs.DVLVelocityTracking',
        ros_type='dave_interfaces/msg/DVL',
        direction=BridgeDirection.GZ_TO_ROS)
...
def odometry(model_name, sensor_type):
    return Bridge(
        gz_topic=f'/{model_name}/{sensor_type}',
        ros_topic=f'/{model_name}/{sensor_type}',
...
def ball_shooter(model_name):
    return Bridge(
        gz_topic=f'/{model_name}/ball_shooter/fire',
        ros_topic=f'ball_shooter/fire',
...
def shooter_pan_joint_pos(model_name):
    # ROS naming policy indicates that first character of a name must be an alpha
    # character. In the case below, the gz topic has the joint index 0 as the
    # first char so the following topics fail to be created on the ROS end
    # left_joint_topic = '/model/' + model_name + '/joint/left_chasis_engine_joint/0/cmd_pos'
    ...
    # For now, use erb to generate unique topic names in model.sdf.erb
    return Bridge(
        gz_topic=f'{model_name}/ball_shooter/pan',
        ros_topic=f'ball_shooter/pan',
        gz_type='gz.msgs.Double',
        ros_type='std_msgs/msg/Float64',
        direction=BridgeDirection.ROS_TO_GZ)

def shooter_tilt_joint_pos(model_name):
    # ROS naming policy indicates that first character of a name must be an alpha
    # character. In the case below, the gz topic has the joint index 0 as the
    # first char so the following topics fail to be created on the ROS end
    # left_joint_topic = '/model/' + model_name + '/joint/left_chasis_engine_joint/0/cmd_pos'
    ...
    # For now, use erb to generate unique topic names in model.sdf.erb
    return Bridge(
        gz_topic=f'{model_name}/ball_shooter/tilt',
        ros_topic=f'ball_shooter/tilt',
        gz_type='gz.msgs.Double',
        ros_type='std_msgs/msg/Float64',
        direction=BridgeDirection.ROS_TO_GZ)
...
def payload_bridges(world_name, model_name, link_name, sensor_name, sensor_type):
    bridges = []
    print(f"sensor_name {sensor_name} link_name {link_name} sensor-type {sensor_type}")
    ...
    elif 'OdometryPublisher' in link_name:
        bridges = [
            odometry(model_name, sensor_type),
        ]
    elif 'BallShooter' in sensor_name:
        bridges = [
            ball_shooter(model_name),
            shooter_pan_joint_pos(model_name),
            shooter_tilt_joint_pos(model_name),
        ]
    elif 'thruster_thrust' in sensor_name:
        bridges = [
            thrust(model_name, sensor_type),
        ]
    elif 'thruster_rotate' in sensor_name:
        bridges = [
