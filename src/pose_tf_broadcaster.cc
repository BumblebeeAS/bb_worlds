...
class FramePublisher : public rclcpp::Node
{
  ...
  private: void HandlePose(const std::shared_ptr<tf2_msgs::msg::TFMessage> _msg)
  {
    ...
    if (!world_frame.empty())
    {
      ...
      {
        std::cout << "transform.header.frame_id: " << transform.header.frame_id << std::endl;
    ...
    {
      std::vector<geometry_msgs::msg::TransformStamped> transformed;
      for (auto transform: _msg->transforms)
      {
        // remove transform.header.frame_id from transform.child_frame_id
        std::string child_frame_id = transform.child_frame_id;
        std::string::size_type pos = child_frame_id.find(transform.header.frame_id);
        if (pos != std::string::npos)
        {
          child_frame_id.erase(pos, transform.header.frame_id.length());
        }
        //add first part of transform.header.frame_id before / to transform.child_frame_id
        pos = transform.header.frame_id.find("/");
        if (pos != std::string::npos)
        {
          child_frame_id.insert(0, transform.header.frame_id.substr(0, pos));
        }
        // std::cout << "transform.child_frame_id: " << transform.child_frame_id << 
        //   " transform.header.frame_id: " << transform.header.frame_id <<
        //   " child_frame_id: " << child_frame_id <<         
        // std::endl;
        transform.child_frame_id = child_frame_id;
        transformed.push_back(transform);
      }

      // Send the transformation
      this->tfBroadcaster->sendTransform(transformed);
    }
  }
  ...
  private: void HandlePoseStatic(const std::shared_ptr<tf2_msgs::msg::TFMessage> _msg)
  {
    ...
    else
    {
      ...
      for (auto transform: _msg->transforms)
      {
        ...
        // std::cout << "static: transform.child_frame_id: " << transform.child_frame_id << 
        //   " transform.header.frame_id: " << transform.header.frame_id <<
        //   " child_frame_id: " << child_frame_id <<         
        // std::endl;
        transform.child_frame_id = child_frame_id;
      ...
      // Send the transformation
      this->tfBroadcasterStatic->sendTransform(transformed);
    }
  }
